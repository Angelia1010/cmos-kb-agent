...
服务定位:
    检索→处理→验证 Agent Loop(不拼接对话历史、不做答案生成):
      RetrievalSubAgent 经 uniagent create_agent() 组装 —
        ① ReAct 自主调用检索工具(intergrate_all / query_rewrite /
    ...
    另提供 /keyword、/vector 单路直调端点与 /retrieve/batch 批量召回率测试端点。
...
def _default_es(model: Any = None) -> ESClient:
    ...
    缺省生产 ngkm ProduceESClient(构造本身不发网络请求,故不做异常回退;
    真实失败发生在 keyword_search/vector_search 调用时,由 runner/app 的
    异常路径处理)。model 注入供 _extract_keywords 使用。
    RETRIEVAL_SERVICE_BACKEND=mock 时回退离线 MockESClient(内置样例)。
    """
    if os.environ.get(ENV_BACKEND, "").strip().lower() == "mock":
        logger.warning("%s=mock: 使用离线 MockESClient(仅内置样例)", ENV_BACKEND)
        return MockESClient()
