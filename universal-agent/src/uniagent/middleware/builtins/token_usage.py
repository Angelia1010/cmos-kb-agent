...
class TokenUsageMiddleware(Middleware):
    ...
    def loop_hooks(self) -> list[Any]:
        """暴露 TokenBudgetHook，将 token 用量同步到循环层预算系统。"""
        if self._budget is not None:
...
def _extract_usage(msg: AIMessage) -> dict[str, int] | None:
    """从 AIMessage 中提取 token 用量字典。

    按优先级尝试：usage_metadata（新版 LangChain）→ response_metadata（旧版）。
    """
