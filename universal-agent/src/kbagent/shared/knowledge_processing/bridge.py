...
def _chunk_to_candidate(index: int, chunk: Chunk) -> Dict[str, Any]:
    ...
    # 候选正文:结构化 ngkm 条目走 extra["atoms"](已映射进 atoms,由 markdown
    # 渲染),raw vector 条目正文在 chunk.content;其余 legacy chunk 保持空正文,
    # 由 has_renderable_candidate_content 过滤(processing-dev 契约)。
    return {
