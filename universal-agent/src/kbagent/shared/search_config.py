...
# ── 省份/地市名 → 区号 映射 ─────────────────────────────────────────────────
# 全量省份地市编码映射表,数据源:《知识库统一检索接口V1.6.9-20260622》§6
# 省份地市编码列表(37 省份 + 3473 地市)。由 scripts/gen_region_codes.py
# 生成于 region_codes.py,版本升级时重新生成即可。
# 检索阶段透传约定: 传入值已是编码(或表外未知值)时原样透传,
# 仅可唯一换算的名称(省份名/无歧义地市名)才替换为编码。
from .region_codes import REGION_NAME_TO_CODE as _PROVINCE_TO_REGION  # noqa: E402
