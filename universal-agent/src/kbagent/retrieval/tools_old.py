...
安全边界:LLM 只传结构化参数,coarse_recall 内部经 keyword_search 流水线召回,
LLM 永远不接触 ES DSL。
"""
...
def coarse_recall(relax_filters: bool = False, retrieval_mode: str = "hybrid") -> str:
    ...
    # search.py 已移除 build_dsl / rrf_fuse,coarse_recall 改为直接走 keyword_search 流水线:
    qtext = ws.data.get("rewritten_query") or ws.query
    region_code = ws.data.get("region_code", "000")
    result = ws.es.keyword_search(query=qtext, region_code=region_code) \
        if retrieval_mode in ("keyword", "hybrid") else {"merged": []}
    merged = result.get("merged", []) if isinstance(result, dict) else []
    khits: List[Chunk] = merged_to_chunks(merged)
    vhits: List[Chunk] = (ws.es.vector_search(qtext, {"region": region_code}, ws.cfg.recall_size)
                          if retrieval_mode in ("vector", "hybrid") else [])
    fused: List[Chunk] = (khits + vhits)[: ws.cfg.fuse_top_n]
...
def intergrate_all(query: str = "", region_code: str = "000",
    ...
    #     logger.warning("intergrate_all: 当前检索后端 %s 无 keyword_search,"
