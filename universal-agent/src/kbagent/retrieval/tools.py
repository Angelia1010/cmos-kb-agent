...
intergrate_all(keyword+vector 双路去重)为主路径;keyword_recall / vector_recall
为单路直调(单路排障/服务端点用);query_rewrite 使用 LLM 对上一轮关键词进行
重写,产出更利于检索的改写关键词(供 intergrate_all 的 keywords 参数二次召回)。
"""
...
def intergrate_all(query: str = "", region_code: str = "000",
    ...
    if keyword_search is not None:
        ...
        if not kmerged and isinstance(kresult, dict) and kresult.get("error"):
            ...
            # 条目列表在 merged 字段(与 keyword_recall 一致);整包 dict 传入
            # kresult_to_chunks 会因迭代出字符串 key 而静默产出 0 条。
            kchunks = kresult_to_chunks(kmerged)
    else:
...
def vector_recall(query: str = "", region_code: str = "000", vector_mode: str = "both") -> str:
    ...
    if vector_search is None:
        ...
        return _obs(error="当前检索后端不支持向量召回,请改用 keyword_recall")
    query = query or ws.query
...
def keyword_recall(query: str = "", region_code: str = "000",
    ...
    if keyword_search is None:
        ...
        return _obs(error="当前检索后端不支持一体化流水线,请改用 vector_recall")
    query = query or ws.query
