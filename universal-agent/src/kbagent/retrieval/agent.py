# -*- coding: utf-8 -*-
"""检索子智能体(两个形态,当前启用直调形态):

- ``RetrievalSubAgent``(当前启用)— 直调形态:一次性直接调用 tools.py 的
  intergrate_all 工具(槽位提取 → 知识主索引召回 → 原子表拼接),并传入
  省份信息 region_code(省份名或区号均可,如 福建/591/000,由
  shared.search 内部归一为区号);全程零 LLM 参与,不走 agent loop。
  降级护栏:检索后端不支持一体化流水线(如离线 MockESClient)或流水线报错时,
  确定性退化为 keyword_extraction → coarse_recall;兜底仍零召回且报错 →
  显式失败,交由主智能体降级。
- ReAct 形态(暂时注释归档)— LLM 看着检索工具集自主决定"调哪个工具、
  传什么参数、何时结束";护栏:轮次与时间上限(Budget)、充分性判定
  (SufficiencyVerifier)、DSL 字段白名单(coarse_recall 工具内部)。
"""
